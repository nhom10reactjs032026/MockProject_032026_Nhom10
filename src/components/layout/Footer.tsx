      ...
      <div className="container mx-auto">
        <div className="flex flex-col lg:flex-row justify-between items-start gap-12 pb-20 border-b border-border/20">
          <div className="space-y-4">
            <Link to="/" className="text-4xl font-bold text-foreground uppercase block mb-2 hover:text-primary transition-colors cursor-default">WolfArch</Link>
            <span className="text-[10px] font-bold text-primary tracking-[0.5em] block uppercase opacity-70">Premium Notary Services USA</span>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-12">
            <span className="text-xs font-bold uppercase tracking-widest opacity-40">Kết nối cộng đồng</span>
...
const FooterLink = ({ label }: { label: string }) => (
  <a href="#" className="text-xs font-bold text-muted-foreground hover:text-primary transition-all tracking-wider uppercase opacity-80 hover:opacity-100">
    {label}
  </a>
);
...
const ContactItem = ({ icon, text }: { icon: React.ReactNode, text: string }) => (
  <div className="flex items-center gap-4 group cursor-pointer">
    ...
    <span className="text-[11px] font-bold text-muted-foreground group-hover:text-foreground transition-colors">{text}</span>
  </div>
);

const SocialIcon = ({ icon }: { icon: React.ReactNode }) => (
  <a href="#" className="w-12 h-12 rounded-full bg-secondary hover:bg-white text-muted-foreground hover:text-primary flex items-center justify-center transition-all shadow-sm hover:shadow-xl transform hover:scale-110 active:scale-95">
    {icon}
  </a>
);
