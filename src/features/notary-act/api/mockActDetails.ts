...
export const saveCertificateToMockDB = (
  actId: string,
  payload: {
    venue: string;
    date: string;
    signers: string[];
    sealType: "physical" | "electronic";
  },
) => {
  const existing = mockActDetails[actId];

  if (!existing) return null;

  mockActDetails[actId] = {
    ...existing,
    venue: payload.venue,
    date: payload.date,
    ...
    additionalNotes: `Seal type: ${payload.sealType}`,
  };

  return mockActDetails[actId];
};
