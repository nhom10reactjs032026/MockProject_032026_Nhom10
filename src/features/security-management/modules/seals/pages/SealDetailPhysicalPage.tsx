    ...
    <AppShell>
      ...
      {!seal ? (
        ...
        <div className="mt-4 grid gap-4">
          <Card className="p-5">
            <div className="text-lg font-semibold">Physical Seal ID</div>
            <div className="mt-2 flex flex-wrap items-center gap-3">
              <div className="text-xl font-bold">P_{seal.id}</div>
          ...
          <Card className="p-5">
            ...
            <div className="mt-3 grid gap-4 md:grid-cols-2 text-sm">
              <div>
                ...
                <div className="font-semibold">{seal.physical?.shape ?? "—"}</div>
              </div>
              <div>
                <div className="text-xs text-slate-500">Size</div>
                <div className="font-semibold">{seal.physical?.size ?? "—"}</div>
              </div>
              <div>
                <div className="text-xs text-slate-500">Type</div>
                <div className="font-semibold">{seal.physical?.materialType ?? "—"}</div>
              </div>

              <div className="md:col-span-2">
                <div className="text-xs text-slate-500">Seal Impression</div>

                {seal.physical?.impressionImageUrl ? (
                  <img
                    className="mt-2 h-40 w-full rounded-xl border object-contain"
                    src={seal.physical.impressionImageUrl}
                    alt="Seal impression"
                  />
                ) : (
                  <div className="mt-2 h-40 rounded-xl border bg-slate-50 p-4 text-xs text-slate-500">
                    {seal.physical?.impressionText ?? "(No impression)"}
                  </div>
                )}
              </div>
            </div>
